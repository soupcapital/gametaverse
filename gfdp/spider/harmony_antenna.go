...
type HarmonyAntenna struct {
	node    string
	chainID int
}

func NewHarmonyAntenna() *HarmonyAntenna {
	antenna := &HarmonyAntenna{}
	return antenna
}

func (ata *HarmonyAntenna) Init(rpc string, chainID int) (err error) {
...
func (ata *HarmonyAntenna) GetBlockHeight(ctx context.Context) (height uint64, err error) {
	//rsp, err := rpc.Request(rpc.Method.BlockNumber, ata.node, []interface{}{})
	if err != nil {
		return
	}
	//log.Info("rsp is %v", rsp)
	return
}

func (ata *HarmonyAntenna) GetTrxByNum(ctx context.Context, num uint64) (trxes []*Transaction, err error) {
...
func (ata *HarmonyAntenna) DealTrx(rawtrx *Transaction) (txes []*db.Transaction, err error) {
